...
-(void)insertOfFMWithDataArray:(NSArray *)dataArr{
    if ([_dataBase open]) {
        [dataArr enumerateObjectsUsingBlock:^(NSDictionary *  _Nonnull model, NSUInteger index, BOOL * _Nonnull stop) {
            NSString *insertSql= [NSString stringWithFormat:@"INSERT INTO NEW_LIST('%@','%@','%@','%@','%@','%@','%@')VALUES('%@','%@','%@','%@','%@','%@','%@')",@"headPic",@"userName",@"phone",@"password",@"groupID",@"groupName",@"nickName",model[@"headPic"],model[@"userName"],model[@"phone"],model[@"password"],model[@"groupID"],model[@"groupName"],model[@"nickName"]];
...
-(NSArray *)queryAllDataOfFMDB{
    ...
    if ([_dataBase open]) {
        ...
        while ([resultSet next]) {
            ...
            NSDictionary *dic = @{@"headPic":headPic,
                                  @"userName":userName,
                                  @"phone":phone,
                                  @"password":password,
                                  @"groupID":groupID,
