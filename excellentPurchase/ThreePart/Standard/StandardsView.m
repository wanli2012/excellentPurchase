...
//    self.mainImgView.alpha = 0;
    [self addSubview:self.mainImgView];
        /*默认按键*/
    cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, StandardViewHeight-50, StandardViewWidth/2, 50)];
...
//
    sureBtn = [[UIButton alloc] initWithFrame:CGRectMake(StandardViewWidth/2, StandardViewHeight-50, StandardViewWidth/2, 50)];
    sureBtn.tag = 1000 + 2;
...
-(void) initTableView
{
    UIView *tempView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 110)];
    //最上面显示的信息 数据
    self.tipLab.frame = CGRectMake(self.mainImgView.frame.size.width+self.mainImgView.frame.origin.x+10,
                                     0,
    ...
    self.priceLab.frame =CGRectMake(self.tipLab.frame.origin.x,
                                   self.tipLab.frame.origin.y+self.tipLab.frame.size.height,
                                   self.tipLab.frame.size.width, 25);
    ...
    self.goodNum.frame = CGRectMake(self.priceLab.frame.origin.x,
                                   ...
                                   self.priceLab.frame.size.width, 25);
    self.goodNum.textColor = [UIColor blackColor];
    self.goodNum.font = [UIFont systemFontOfSize:15];
...
-(void)setCustomBtns:(NSArray *)customBtns
{
    ...
    for (int i = 0; i < _customBtns.count; i++) {
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(btnWidth*i, StandardViewHeight-btnHeight, btnWidth, btnHeight)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //        NSString *strID = [NSString stringWithFormat:@"%@",dicGoodsDetail[@"spec_name"][indexPath.row][@"id"]];
        ...
        else
        {
            ...
            plusBtn.titleLabel.font = [UIFont systemFontOfSize:18];
            plusBtn.backgroundColor = MAIN_COLOR;
            [cell addSubview:plusBtn];
            
            self.numberTextFied.frame = CGRectMake(plusBtn.frame.origin.x - 40, plusBtn.origin.y, 40, 30);
            self.numberTextFied.textAlignment = NSTextAlignmentCenter;
            self.numberTextFied.backgroundColor = [UIColor whiteColor];
            self.numberTextFied.layer.borderWidth = 0.5;
            self.numberTextFied.layer.borderColor = YYSRGBColor(251, 160, 163, 1).CGColor;
            ...
            [reduceBtn addTarget:self action:@selector(buyNumBtnClick:) forControlEvents:UIControlEventTouchUpInside];
             reduceBtn.backgroundColor = YYSRGBColor(251, 160, 163, 1);
            [plusBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
