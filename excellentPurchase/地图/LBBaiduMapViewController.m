...
- (void)viewDidLoad {
    ...
    UIView *titleView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, UIScreenWidth - 200, 35)];
    [titleView setBackgroundColor:[UIColor clearColor]];

    UISearchBar *searchBar = [[UISearchBar alloc] init];
    searchBar.delegate = self;
    searchBar.barTintColor = [UIColor clearColor];
    searchBar.frame = CGRectMake(0, 0, UIScreenWidth - 200, 35);
    searchBar.layer.cornerRadius = 17;
    searchBar.backgroundColor = [UIColor clearColor];
    searchBar.layer.masksToBounds = YES;
    searchBar.placeholder = @"点击搜索地址";
...
{
    [EasyShowTextView showErrorText:@"定位失败"];
}
...
- (void)customLocationAccuracyCircle {
    ...
    if (_isGetLocation) {
        ...
        [EasyShowTextView showErrorText:@"还未定位到当前位置"];
    }
...
-(BMKMapView*)mapView{
    if (!_mapView) {
        _mapView = [[BMKMapView alloc]initWithFrame:CGRectMake(0, 0, UIScreenWidth, UIScreenHeight)];
