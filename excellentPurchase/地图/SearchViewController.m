...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *navigaH;
...
- (void)viewDidLoad {
    [super viewDidLoad];
   UIView *serachView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, UIScreenWidth - 150, 30)];
    serachView.backgroundColor = [UIColor groupTableViewBackgroundColor];
    serachView.layer.cornerRadius = 4;
    serachView.clipsToBounds = YES;
    
    self.navigaH.constant = SafeAreaTopHeight;
    _inputAddTF = [[UITextField alloc] initWithFrame:CGRectMake(10, 0, UIScreenWidth - 130, 30)];
    ...
    imageViewPwd.image=[UIImage imageNamed:@"search"];
...
//    self.inputAddTF.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    ...
    self.inputAddTF.backgroundColor = [UIColor clearColor];
    
    [serachView addSubview:self.inputAddTF];
    
    self.navigationItem.titleView = serachView;
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField{
    if (textField.text.length <= 0 ) {
        [EasyShowTextView showErrorText:@"请输入搜索关键字"];
        return NO;
    }
