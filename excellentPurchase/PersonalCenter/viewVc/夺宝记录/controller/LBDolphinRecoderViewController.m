//
//  LBDolphinRecoderViewController.m
//  excellentPurchase
//
...
#import "LBDolphinRecoderAlreadyViewController.h"
#import "SPPageMenu.h"
#import "LBShowWinningPictureController.h"

#define pageMenuH 50   //菜单高度

@interface LBDolphinRecoderViewController ()<SPPageMenuDelegate,UIScrollViewDelegate>
...
@implementation LBDolphinRecoderViewController
...
-(void)showPictureEvent{
    
    self.hidesBottomBarWhenPushed = YES;
    LBShowWinningPictureController *vc = [[LBShowWinningPictureController alloc]init];
...
-(NSArray*)menuArr{
    ...
    if (!_menuArr) {
        _menuArr = [NSArray arrayWithObjects:@"全部",@"进行中",@"已开奖",nil];
    }
...
-(NSMutableArray*)controllerClassNames{
    ...
    if (!_controllerClassNames) {
        _controllerClassNames = [NSMutableArray arrayWithObjects:@"LBDolphinRecoderAllViewController",@"LBDolphinRecoderDoingViewController",@"LBDolphinRecoderAlreadyViewController", nil];
    }
...
-(UIButton*)rightBt{
    ...
    if (!_rightBt) {
        ...
        _rightBt.frame = CGRectMake(0, 0, 60, 44);
        _rightBt.backgroundColor = [UIColor clearColor];
        [_rightBt addTarget:self action:@selector(showPictureEvent) forControlEvents:UIControlEventTouchUpInside];
        [_rightBt setContentHorizontalAlignment:UIControlContentHorizontalAlignmentRight];
        [_rightBt setTitle:@"晒图" forState:UIControlStateNormal];
