...
#import "LBMine_PayActivitySucessController.h"//付款状态
#import "HHPayPasswordView.h"

@interface LBindiana_PayController ()<UITableViewDelegate,UITableViewDataSource,HHPayPasswordViewDelegate>
...
@property (weak, nonatomic) IBOutlet UILabel *ordernumbers;//订单号

@property (nonatomic, strong)NSString *payStr;

@end

@implementation LBindiana_PayController
...
- (void)viewDidLoad {
    ...
    self.orderAllPrice.text = [NSString stringWithFormat:@"¥ %@",self.datadic[@"order_price"]];
    self.ordernumbers.text = [NSString stringWithFormat:@"%@",self.datadic[@"order_num"]];
...
-(void)pushsucessVc{
    ...
    LBMine_PayActivitySucessController *vc =[[LBMine_PayActivitySucessController alloc]init];
...
-(void)alipayMethod{
    ...
    dic[@"indiana_order_id"] = self.datadic[@"order_id"];
    ...
    [NetworkManager requestPOSTWithURLStr:kPayindiana_pay paramDic:dic finish:^(id responseObject) {
