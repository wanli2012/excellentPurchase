...
- (void)viewDidLoad {
    ...
    // 添加3个子控制器
    [self addChildViewController:[[LBHomeOneDolphinAllViewController alloc] init]];
    [self addChildViewController:[[LBHomeOneDolphinIngViewController alloc] init]];
    [self addChildViewController:[[LBHomeOneDolphinWaitViewController alloc] init]];
    ...
    // 监听子控制器发出的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(subScrollViewDidScroll:) name:ChildScrollViewDidScrollNSNotificationDolphin  object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshState:) name:ChildScrollViewRefreshStateNSNotificationDolphin object:nil];
...
- (void)subScrollViewDidScroll:(NSNotification *)noti {
    ...
    LBHomeOneDolphinBaseViewController *baseVc = self.childViewControllers[self.pageMenu.selectedItemIndex];
...
- (void)followScrollingScrollView:(UIScrollView *)scrollingScrollView distanceY:(CGFloat)distanceY{
    LBHomeOneDolphinBaseViewController *baseVc = nil;
...
- (void)pageMenu:(SPPageMenu *)pageMenu itemSelectedFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex {
    ...
    LBHomeOneDolphinBaseViewController *targetViewController = self.childViewControllers[toIndex];
    // 如果已经加载过，就不再加载
...
- (SPPageMenu *)pageMenu{
    ...
    if (!_pageMenu) {
        ...
        [_pageMenu setItems:@[@"全部夺宝",@"正在夺宝",@"等待开奖"] selectedItemIndex:0];
