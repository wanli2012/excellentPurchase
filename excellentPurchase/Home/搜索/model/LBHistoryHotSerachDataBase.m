...
@implementation LBHistoryHotSerachDataBase

+(LBHistoryHotSerachDataBase*)greateTableOfFMWithTableName:(NSString *)tableName{
    LBHistoryHotSerachDataBase *dataPeristent = [[LBHistoryHotSerachDataBase alloc]init];
    //创建数据库对象
...
-(void)insertOfFMWithDataArray:(NSArray *)dataArr{
    if ([_dataBase open]) {
        [dataArr enumerateObjectsUsingBlock:^(NSString *  _Nonnull model, NSUInteger index, BOOL * _Nonnull stop) {
            NSString *insertSql= [NSString stringWithFormat:@"INSERT INTO NEW_LIST('%@')VALUES('%@')",@"recoder",model];
