//
//  LBTomrrowTimeLimitViewController.m
//  excellentPurchase
//
...
@interface LBTomrrowTimeLimitViewController ()<UITableViewDelegate,UITableViewDataSource,TXScrollLabelViewDelegate>
...
@implementation LBTomrrowTimeLimitViewController
...
-(void)countDown{
    ...
    [[NSNotificationCenter defaultCenter]postNotificationName:TimeCellNotificationone object:nil userInfo:@{@"index":@"2"}];
}
//请求数据
...
-(UIView*)tableView:(UITableView*)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (self.raceArr.count <= 0) {
        if (self.model.buying.data.count > 0) {
            if (section == 0) {
                LBTimeLimitTomrroeHeaderView *headerview = [tableView dequeueReusableHeaderFooterViewWithIdentifier:@"LBTimeLimitHeaderView"];
                if (!headerview) {
                    headerview = [[LBTimeLimitTomrroeHeaderView alloc] initWithReuseIdentifier:@"LBTimeLimitHeaderView"];
                }
...
-(void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section{
    if (self.raceArr.count <= 0) {
        if (self.model.buying.data.count > 0) {
            if (section == 0) {
                LBTimeLimitTomrroeHeaderView  *headerview =  (LBTimeLimitTomrroeHeaderView*)view;
