//
//  LBSnapUpDetailViewController.m
//  excellentPurchase
//
...
#import "LBSnapUpDetailViewMarkCell.h"
#import "LBSnapUpDetailViewTableViewCell.h"
#import "LBSnapUpDetailViewAwardCell.h"
#import "LBSnapUpDetailViewSpecificationCell.h"
...
@interface LBSnapUpDetailViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,LBCheckMoreCommentsDelegate,StandardsViewDelegate>

@property(nonatomic,strong)DLNavigationTabBar *navigationTabBar;
@property (weak, nonatomic) IBOutlet UIButton *merchetBt;//商家
@property (weak, nonatomic) IBOutlet UIButton *collectBt;//收藏
@property (weak, nonatomic) IBOutlet UIButton *serviceBt;//客服
...
@property (nonatomic, strong) StandardsView *mystandardsView;//加载商品规格
...
@property (nonatomic ,assign) NSInteger ptype;//商品类型 1厂家直销 2产地直供 3品牌加盟 4渠道授权/微商特供 5商场自营 6顺德商城
/**
 评论view
 */
...
static NSString *snapUpDetailViewMarkCell = @"LBSnapUpDetailViewMarkCell";
static NSString *snapUpDetailViewTableViewCell = @"LBSnapUpDetailViewTableViewCell";
static NSString *snapUpDetailViewAwardCell = @"LBSnapUpDetailViewAwardCell";
static NSString *snapUpDetailViewSpecificationCell = @"LBSnapUpDetailViewSpecificationCell";
...
@implementation LBSnapUpDetailViewController
...
- (void)viewDidLoad {
    ...
    [self.serviceBt verticalCenterImageAndTitle:5];
...
#pragma mark - 立即购买
- (IBAction)ImmediatelyBuy:(UIButton *)sender {
    [self chooseSpecification];
}
#pragma mark - 进入商家
- (IBAction)jionmerchat:(UIButton *)sender {
...
#pragma mark - 收藏
- (IBAction)collectionEevnt:(UIButton *)sender {
...
-(void)registerTablecell{
    // 注册cell
    [self.tableview registerNib:[UINib nibWithNibName:snapUpDetailViewMarkCell bundle:nil] forCellReuseIdentifier:snapUpDetailViewMarkCell];
    [self.tableview registerNib:[UINib nibWithNibName:snapUpDetailViewTableViewCell bundle:nil] forCellReuseIdentifier:snapUpDetailViewTableViewCell];
    [self.tableview registerNib:[UINib nibWithNibName:snapUpDetailViewAwardCell bundle:nil] forCellReuseIdentifier:snapUpDetailViewAwardCell];
    [self.tableview registerNib:[UINib nibWithNibName:snapUpDetailViewSpecificationCell bundle:nil] forCellReuseIdentifier:snapUpDetailViewSpecificationCell];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            if ([self.model.bonuspoints floatValue] <= 0 && [self.model.reword_coupons floatValue] <= 0 ) {
                return 0;
            }else{
...
-(UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            LBSnapUpDetailViewTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:snapUpDetailViewTableViewCell forIndexPath:indexPath];
            ...
            LBSnapUpDetailViewMarkCell *cell = [tableView dequeueReusableCellWithIdentifier:snapUpDetailViewMarkCell forIndexPath:indexPath];
            cell.selectionStyle = 0;
            return cell;
        }else if (indexPath.row == 2){
            LBSnapUpDetailViewAwardCell *cell = [tableView dequeueReusableCellWithIdentifier:snapUpDetailViewAwardCell forIndexPath:indexPath];
            cell.selectionStyle = 0;
            if ([self.model.bonuspoints floatValue] <= 0 && [self.model.reword_coupons floatValue] <= 0 ) {
                cell.hidden = YES;
            }else{
            ...
            LBSnapUpDetailViewSpecificationCell *cell = [tableView dequeueReusableCellWithIdentifier:snapUpDetailViewSpecificationCell forIndexPath:indexPath];
            cell.selectionStyle = 0;
            return cell;
        }
    }else if (indexPath.section == 1){
...
-(StandardsView *)buildStandardView:(NSString *)img andIndex:(NSInteger)index
{
    ...
    standview.priceLab.text = [NSString stringWithFormat:@"¥%@",((LBTmallProductDetailgoodsSpecModel*)self.model.goods_spec[0]).costprice];
...
-(void)Standards:(StandardsView *)standardView SelectBtnClick:(UIButton *)sender andSelectID:(NSString *)selectID andStandName:(NSString *)standName andIndex:(NSInteger)index
{
    ...
    if (self.model.goods_spec.count != 0) {
        ...
        if ([NSString StringIsNullOrEmpty:model1.marketprice] == NO) {
            _mystandardsView.priceLab.text = [NSString stringWithFormat:@"¥%@",model1.costprice];
        }
...
-(SDCycleScrollView*)cycleScrollView{
    ...
    if (!_cycleScrollView) {
        _cycleScrollView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 0, UIScreenWidth, UIScreenWidth) delegate:nil placeholderImage:[UIImage imageNamed:@"shangpinxiangqing"]];//当一张都没有的时候的 占位图
